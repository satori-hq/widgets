...
export function forceSimulation<ND extends N, LinkDatum extends L<ND>>(
  nodesData?: ND[]
): Simulation<ND, LinkDatum>;

export interface Force<ND extends N, LinkDatum extends L<ND> | undefined> {
  (alpha: number): void;
...
export interface ForceLink<ND extends N, LinkDatum extends L<ND>>
  extends Force<ND, LinkDatum> {
  initialize(nodes: ND[], random: () => number): void;
  links(): LinkDatum[];
